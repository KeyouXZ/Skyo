    ...
    if (args[0] === 'wallet') {
      ...
      const leaderboard = [];
        ...
        for (let i = 0; i < dataWallet.length && i < 10; i++) {
          ...
          leaderboard.push(`\`${i + 1}.\` ${user?.tag ?? 'Unknown User'} — **${bot.config.currency}${dataWallet[i].wallet.toLocaleString()}**`);
        }
      ...
      for (let i = 0; i < dataBank.length && i < 10; i++) {
        ...
        leaderboard.push(`\`${i + 1}.\` ${user?.tag ?? 'Unknown User'} — **${bot.config.currency}${dataBank[i].bank.toLocaleString()}**`);
      }
