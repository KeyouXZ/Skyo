...
module.exports = {
    ...
    run: async (client, message, args) => {
        ...
        if (!target) {
            return util.tempMessage(message, "Please mention a user");
        }
        ...
        if (target.user.id == message.author.id) {
            ...
            return util.tempMessage(message, "You can't give money to bot");
        }
        ...
        if (isNaN(amount)) {
            return util.tempMessage(message, "Please specify a valid amount.");
        }
        if (
            args[1].includes("-") ||
            args[1].includes("+") ||
